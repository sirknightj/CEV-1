	...
	for building in buildings:
		if (building.building_id == GameData.BuildingType.FOOD1
			...
			for adjacent in building.get_adjacent_buildings():
				if (adjacent.building_id == GameData.BuildingType.WATER1
						or adjacent.building_id == GameData.BuildingType.WATER2
						or adjacent.building_id == GameData.BuildingType.WATER3):
					found = true
