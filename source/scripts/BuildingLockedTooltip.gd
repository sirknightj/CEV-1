...
func show_building(building : Building):
	...
	if missing_text.empty() or not GameStats.restrictions.empty() or GameStats.turn < 6 or (GameStats.win_status and building.building_id == GameData.BuildingType.END1):
